...
class McqController extends Controller
{
    ...
    public function mcqOgImage(Mcq $mcq, Request $request)
    {
        $request->validate([
            'data.action' => 'required|in:generate,regenerate',
        ]);

        $action = $request->input('data.action');

        GenerateMcqOgImageJob::dispatch($mcq, $action);

        return back()->with('success', 'Image generation started 🚀 ' . ucfirst($action) . 'ing...');
    }
    ...
    //     if (file_exists($path) && $action === 'generate') {
    ...
    //         $image->text('Explanation:', 600, $explanationHeadingY, function ($font) use ($explanationTextSize, $explanationWrap) {
    //             $font->filename(public_path('fonts/Roboto-Bold.ttf'));
    ...
    //         $image->text($mcq->explanation, 600, $explanationY, function ($font) use ($explanationTextSize, $explanationWrap) {
    //             $font->filename(public_path('fonts/Roboto-Bold.ttf'));
    //             $font->size($explanationTextSize);
    //             $font->lineHeight(1.8);
    //             $font->color('#333');
