...
<div x-data="{
    ...
    <div class="mb-2 flex flex-col-reverse md:flex-row md:justify-between gap-2">
        ...
        <div class="flex items-center justify-end space-x-1">
            ...
            @endif
            <button @click="shareLink" class="p-2 hover:bg-gray-100 rounded-full text-gray-500">
                <x-heroicon-o-share class="h-5 w-5" />
            </button>
        </div>
    </div>

    <div class="my-6 text-sm font-semibold lg:text-base">
        <a href="{{ $route }}" class="hover:text-blue-600">
            ...
            <button @click="selectOption({{ $opt->id }})" :disabled="!isQuizMode || wasAnswered"
                :class="{
                    ...
                    'border-red-500 bg-red-100': wasAnswered && selectedOptionId == {{ $opt->id }} && !
                        {{ $opt->is_correct ? 'true' : 'false' }},
                    'border-gray-200 bg-white/60 hover:border-blue-500': !wasAnswered || (isQuizMode && !wasAnswered)
                }"
    ...
    @if ($mcq['explanation'])
        <div x-data="{ open: !isQuizMode }" class="mt-4 border-t pt-2" x-show="wasAnswered || !isQuizMode">
            <button @click="open = !open" class="flex items-center justify-between w-full text-sm font-medium py-2">
                <span>Explanation</span>
                <x-heroicon-o-chevron-down class="h-4 w-4 transform transition-transform"
