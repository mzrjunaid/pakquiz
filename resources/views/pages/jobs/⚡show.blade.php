    ...
    public function with(): array
    {
        $breadcrumbSchema = [
            ...
            'itemListElement' => [
                ...
                ['@type' => 'ListItem', 'position' => 3, 'name' => $this->job->title, 'item' => url('/jobs/' . $this->job->slug)]
            ],
        ];
        ...
        $schema = [
            ...
            ?? "Apply for {$this->job->title} at {$this->job->department->name}. "
             . "Minimum qualification: {$this->job->minimum_qualification}. "
             . "Total posts: {$this->job->total_posts}. "
             . "BPS Scale: {$this->job->scale}. "
             . "Closing date: " . Carbon::parse($this->job->closing_date)->format('d M Y') . ".",
            'datePosted'    => Carbon::parse($this->job->created_at)->toAtomString(),
            'validThrough'  => Carbon::parse($this->job->closing_date)
                                ->endOfDay()
                                ->toAtomString(),
            'hiringOrganization' => [
                '@type'  => 'Organization',
            ...
            'specialCommitments' => $this->job->age_relaxation
            ? 'Age relaxation available: ' . $this->job->age_relaxation
            : null,
