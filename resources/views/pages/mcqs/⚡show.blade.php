        ...
        <nav class="flex mb-5 text-sm" aria-label="{{ __('Breadcrumb') }}">
            <ol class="inline-flex items-center md:space-x-1">
                ...
                <li>
                    <div class="flex items-center">
                        <span class="mx-2">/</span>
                        <span class="font-medium text-primary max-w-xs line-clamp-1 {{ $mcq->isUrdu($mcq->question) ? 'font-urdu direction-rtl text-right' : '' }}">{{ $mcq->question }}</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <section x-data="{
            ...
            <div class="grid gap-6 lg:grid-cols-3 lg:gap-8">
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <h1 class="text-base md:text-2xl font-bold {{ $mcq->isUrdu($mcq->question) ? 'font-urdu direction-rtl text-right' : '' }}" wire:ignore.self title="{{ $mcq->question }}">
                        {{ $mcq->question }}
                    </h1>
                    <div class="grid gap-2 md:grid-cols-1 lg:gap-3 {{ $mcq->isUrdu($mcq->question) ? 'font-urdu direction-rtl text-right' : '' }}">
