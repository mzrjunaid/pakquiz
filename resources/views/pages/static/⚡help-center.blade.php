...
new class extends Component {
    ...
    {
        return [['id' => 'faqs', 'label' => 'FAQs', 'icon' => 'hand-heart-fill'], ['id' => 'getting-started', 'label' => 'Getting Started', 'icon' => 'book-open-fill'], ['id' => 'premium', 'label' => 'Premium Features', 'icon' => 'star-fill'], ['id' => 'jobs', 'label' => 'Jobs & Updates', 'icon' => 'briefcase-fill'], ['id' => 'billing', 'label' => 'Account & Billing', 'icon' => 'bank-card-fill'], ['id' => 'support', 'label' => 'Contact & Support', 'icon' => 'mail-fill']];
    }
    ...
    public function filteredFaqs(): array
    {
        ...
        return array_values(array_filter($this->faqs, fn($faq) => str_contains(strtolower($faq['question']), $query) || str_contains(strtolower($faq['answer']), $query)));
    }
    ...
    public function gettingStartedGuides(): array
    {
        return [
            [
                ...
                'steps' => ['Click on the "Sign Up" button in the top right corner', 'Enter your name, email address, and create a password', 'Verify your email address by clicking the link sent to your inbox', 'Complete your profile with optional information', 'Start practicing MCQs immediately!'],
            ],
            [
                'title' => 'How to attempt MCQs online',
                'steps' => ['Navigate to the "MCQs Bank" section from the main menu', 'Select a category (General Knowledge, English, Math, etc.)', 'Click on any question to view it', 'Select your answer from the multiple choices provided', 'Click "Submit" to see if your answer is correct', 'Review the explanation (if available) to understand the concept'],
            ],
            [
                'title' => 'How to use demo practice papers',
                'steps' => ['Go to the "Practice Papers" section', 'Browse available demo papers for different exams', 'Click "Start Test" on any demo paper', 'Answer all questions within the time limit', 'Submit your paper to see your score and correct answers', 'No account needed for demo papers!'],
            ],
            [
                'title' => 'How to upgrade to premium membership',
                'steps' => ['Log in to your account', 'Click on "Upgrade to Premium" in your dashboard', 'Choose your subscription plan (monthly or annual)', 'Select your preferred payment method', 'Complete the payment process', 'Your premium features will be activated immediately'],
            ],
            [
                'title' => 'How to reset your password',
                'steps' => ['Click on "Forgot Password?" on the login page', 'Enter your registered email address', 'Check your email for a password reset link', 'Click the link and enter your new password', 'Confirm your new password', 'Log in with your new credentials'],
            ],
        ];
    }
    ...
    {
        return array_merge(SchemaGenerator::website(), SchemaGenerator::helpCenterPage());
    }
};
?>
        ...
        <nav class="flex mb-2 text-sm" aria-label="{{ __('Breadcrumb') }}">
            <ol class="inline-flex items-center md:space-x-1">
                ...
                <li class="inline-flex gap-1 items-center">
                    ...
                    <span class="font-medium text-primary" title="{{ __('Help Center') }}" aria-label="{{ __('Help Center') }}">{{ __('Help Center') }}</span>
                </li>
            </ol>
        </nav>
        <x-page-header title="Help Center"
            description="Find answers to common questions about PakQuiz MCQs preparation platform." />
    </div>
